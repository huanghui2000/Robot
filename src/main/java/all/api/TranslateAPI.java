...
public class TranslateAPI {
    ...
    //获取URL中的mp3文件
    public static long loanVoice(String url) throws Exception {
        long time = System.currentTimeMillis();
        //设定当前时间为图片的关键字
        URL pic = new URL(url);
        String savePath = "image/" + time + ".wav";
        //对图片的各种流操作
    ...
    //对语音翻译的处理
    public static long getVoice(String query, String to) throws Exception {
        URL url = getUrl(query, to);
        String sb = String.valueOf(Basics.getStringBuilder(url, 0));
        JSONObject firstDate = JSONObject.fromObject(sb);
        String voiceUrl = String.valueOf(firstDate.get("tSpeakUrl"));
        return loanVoice(voiceUrl);
    }
    ...
    public static String getCH(String query, String to) throws Exception {
        ...
        return String.valueOf(firstDate.getJSONArray("translation").get(0));
    }

    //获取有道的翻译URL
    public static URL getUrl(String query, String to) throws Exception {
        ...
        return new URL(top + "?q=" + URLEncoder.encode(query, "utf-8") + "&from=" + from + "&to=" + to + "&salt=" + salt + "&appKey=" + appKey + "&signType=" + signType + "&curtime=" + curTime + "&sign=" + sign + "&ext=" + ext);
    }
    ...
    public static String getLanguage(String said) throws Exception {
        ...
        if (said.startsWith("语音翻译")) {
            isVoice = true;
            said = said.substring(3);
        }
        //提取语种和对应符号集
        ...
        if (said.equals(" ") || said.equals(""))
            said = "对不起没有检测到您的语句";
        if (isVoice)
            return String.valueOf(getVoice(said, to));
        else
            return getTranslate(said, to);
    }
